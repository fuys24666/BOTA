...
def _partitioned(root: Path, config: dict[str, Any], protocol: Path, registry: dict[str, Any], method: str, run_name: str, stage: Path) -> dict[str, Any]:
    kind = "sisa" if method == "SISA" else "receraser"; component_rows = []; started = time.perf_counter(); dataset_extension = str(config.get("source", {}).get("dataset", "")).startswith("amazon_movies_tv_titles_exploratory")
    for scenario in registry["scenarios"]:
        ...
        else: result = partitioned.execute(root, config_path, component_name, kind, False, partition_seed_override=int(config["protocol"]["seed"]), formal=not dataset_extension)
